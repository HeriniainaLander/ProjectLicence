                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box">
                            <h4 class="page-title">Table Diffusion</h4>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Liste des Diffusions disponibles</h3>
                            </div>
                            <div class="card-body">
                                <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                    ...
                                    @foreach($diffusions as $diffusion)
                                        <tr>
                                            <td>{{ $diffusion->id }}</td>
                                            <td>{{ $diffusion->titre }}</td>
                                            <td>{{ $diffusion->debut_diffusion }}</td>
                                            <td>{{ $diffusion->fin_diffusion }} </td>
                                            <td>{{ $diffusion->heure_diffusion }}</td>
                                            <td>{{ $diffusion->nb_diffusion }}</td>
                                            <td>{{ $diffusion->numQuitance }}</td>
                                            <td>
                                                <div class="btn-group mb-2">
                                                    <a href="{{ route('diffusions.edit', $diffusion->id)  }}" class="btn btn-outline-secondary waves-effect btn-sm" rel="tooltip" title="modifier">
                                                        <i class="far fa-edit"></i>
                                                    </a>
                                                    <a href="#" class="btn btn-outline-secondary waves-effect btn-sm delete" data-toggle="modal" data-url="{!! URL::route('diffusions.destroy', $diffusion->id) !!}"  data-name="{{ $diffusion->titre }}" data-target="#card-modal" rel="tooltip" title="supprimer">
